...
import { TitleScreen, PhaseTransitionScreen, GameOverScreen, VictoryScreen } from './screens';
...
export default function App() {
  ...
  // ── Navegação ──
  const startGame = useCallback(() => { setScore(0); setHp(MAX_HP); setScreen('fase1'); }, []);
  const onFase1Complete = useCallback((newScore: number, newHp: number) => { setScore(newScore); setHp(newHp); setScreen('phase_transition'); }, []);
  const startFase2 = useCallback(() => { setScreen('fase2'); }, []);
  const onVictory = useCallback((finalScore: number) => { setScore(finalScore); setScreen('victory'); }, []);
  const onGameOver = useCallback((finalScore: number) => { setScore(finalScore); setScreen('gameover'); }, []);
  ...
  return (
    <div style={{ width: '100vw', height: '100dvh', overflow: 'hidden', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '"Press Start 2P", monospace, system-ui', userSelect: 'none', touchAction: 'none' }} onContextMenu={e => e.preventDefault()}>
      ...
      <div style={{ width: BASE_W, height: BASE_H, transform: `scale(${viewScale})`, transformOrigin: 'center center', position: 'relative', overflow: 'hidden', imageRendering: 'pixelated' }}>
        ...
        {screen === 'fase1' && (
          <Fase1 initialScore={0} initialHp={MAX_HP} muted={muted} onToggleMute={toggleMute} onComplete={onFase1Complete} onGameOver={onGameOver} onRestart={startGame} />
        )}

        {screen === 'fase2' && (
          <Fase2 initialScore={score} initialHp={hp} muted={muted} onToggleMute={toggleMute} onVictory={onVictory} onGameOver={onGameOver} onRestart={startGame} />
        )}
